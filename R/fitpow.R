...
#' @param nmin Minimum allowed value of p.
...
fitpow = function(conc, resp, bidirectional = TRUE, verbose = FALSE, nofit = FALSE, nmin = .3,
  ...
  if(!bidirectional){
    bnds <- c(1e-8*abs(a0), -1e8*abs(a0), # a bounds
              nmin, -20) # p bounds (min of 1 to avoid inf slope at zero)
  } else {
  ...
  if (!is(fit, "try-error")) { # The model fit the data
    if(verbose) cat("power >>>",fit$counts[1],fit$convergence,"\n")
